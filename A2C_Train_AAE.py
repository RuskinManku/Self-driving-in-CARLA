...
class A2C():
    ...
    def run_episodes(self,num_episodes,std_dev=0.5,lr_actor=1e-3,lr_critic=1e-4,lr_autoencoder=1e-6,reward_log_stepsize=100,buffer_size=5,image_size=(480,480,3),gamma=0.99,switch_train_steps=20):
        ...
        # self.policy_network.load_state_dict(torch.load("ae.pth"))
        ...
        for episode in range(num_episodes):
            ...
            while(1):
                ...
                action_stddevs=torch.sigmoid(action_stats[self.action_space_size:]) #sigmoid so our std dev is between 0 and 1 and not negative or too large
                ...
                print(f"Image/{curr_grad_step}_{info}.jpg")
                print("")
...
def main():
    ...
    policy_2,reward_history_2=a2c.run_episodes(num_episodes=1,lr_actor=1e-4,lr_critic=1e-4,lr_autoencoder=1e-6,reward_log_stepsize=1,gamma=0.99)
    # torch.save(policy_2.state_dict(), "policy.pth")
    # np.save('reward_history_2.npy',reward_history_2)
